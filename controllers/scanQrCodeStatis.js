    ...
    try {
        ...
        const payload = {
            ...
            data: {
                ...
                studentName: String(userName), // Kirim data mentah juga ke frontend jika perlu
        ...
        adminFirebase.messaging().send(payload)
            .then((response) => console.log('Notif Dashboard Sukses:', response))
            .catch((error) => console.log('Notif Dashboard Gagal:', error));
...
exports.updateFcmToken = async (req, res) => {
  try {
    const { fcmToken, userType } = req.body; // userType: 'admin' | 'user'
    const profile = req.user?.profile || req.user;
    ...
    const isStudent = role?.toLowerCase?.() === 'siswa' || role === 'student';
    const TargetModel = isStudent ? Student : GuruTendik;

    // 1. Simpan token ke DB
    await TargetModel.update(
      { fcmToken },
      { where: { id } }
    );

    // 2. Subscribe token ke topic school ini
    // Topic ini yang akan di-broadcast oleh scanSelfDoubleQr
    const topic = `school_absensi_${schoolId}`;
    try {
      await adminFirebase.messaging().subscribeToTopic([fcmToken], topic);
      console.log(`[FCM SUBSCRIBE] ID:${id} | Topic: ${topic}`);
    } catch (subErr) {
      // Jangan fail seluruh request jika subscribe gagal
      console.warn(`[FCM SUBSCRIBE WARN]`, subErr.message);
    }

    res.json({
      success: true,
      message: `Token FCM berhasil diperbarui & subscribe ke ${topic}`
    });
